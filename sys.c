...
#include <linux/sched/mm.h>
 ...
 #endif
 #ifndef GET_TSC_CTL
 # define GET_TSC_CTL(a)
 #endif
 #ifndef SET_TSC_CTL
 # define SET_TSC_CTL(a)
 #endif
 #ifndef GET_FP_MODE
...
new = prepare_creds();
 ...
 SYSCALL_DEFINE3(getresuid, uid_t __user *, ruidp, uid_t __user *, euidp, uid_t 
__user *, suidp)
 {
 const struct cred *cred = current_cred();
 int retval;
...
error:
 ...
 SYSCALL_DEFINE3(getresgid, gid_t __user *, rgidp, gid_t __user *, egidp, gid_t 
__user *, sgidp)
 {
 }
 /*
 ...
 */
 ...
 #endif /* CONFIG_COMPAT */
 SYSCALL_DEFINE3(ma201, int, input1, int, input2, int, input3) {
    static int counter = 0; // Counter starts at 0 by default
    if (input1 == 0) {
        if (input3 < 0) {  // if input3 less then 0, then add input 2 to counter
            counter += input2;
        } else {  
            counter = input2 + input3;  // Else, a third argument is given, so 
counter = input2+input3
        }
        printk(KERN_INFO "Counter Value: %d\n", counter); // print the current 
value of counter
    }
    else if (input1 == 1) {  
        counter = 0; // Reset counter
        printk(KERN_INFO "Counter Value: 0\n"); // Print the current value of 
counter
    }
    else {  
        printk(KERN_ERR "The input was invalid\n"); // Print an error message
        return -EINVAL;
    }
    return 0; // Success
 }
